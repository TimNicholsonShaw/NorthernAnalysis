...
plot(test$Time, test$Abundance)

theta.0 <- min(test$Abundance)*0.5

model.0 <- lm(log(Abundance - theta.0) ~Time, data=test)
...
model <- nls(Abundance ~ alpha * exp(beta * Time) + theta, data=test, start=start)

model

plot(test$Time, test$Abundance)
lines(test$Time, predict(model, list(x=test$Time)))
